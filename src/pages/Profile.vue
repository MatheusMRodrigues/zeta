<template>
  <q-page padding class="bg-grey-2">
    ...
    <div class="section-title flex flex-left q-py-sm q-my-md row">
      ...
      />
      <div name="section-title" class="q-mt-sm app-font-medium text-grey-7">
        PERFIL
      </div>
    </div>
    
    <div class="app-font-medium text-red-5">
...
export default {
  ...
  mounted(){
    this.userProfileForm.avatarSrc = auth.currentUser.photoURL
    this.userProfileForm.username = auth.currentUser.displayName
    this.userProfileForm.role = this.userData.role
    this.userProfileForm.course = this.userData.course
  },
  ...
  methods: {
    ...
    submitForm () {
      ...
      if(!this.$refs.userName.hasError){
        this.updateUser(this.userProfileForm)
      }
      else{
